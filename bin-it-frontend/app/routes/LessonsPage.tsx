...
export default function LessonsPage() {
    ...
    // Logic to find the specific bin image for visual reinforcement
    const getCurrentBin = () => {
        if (!currentLesson) return null;
        const typeMatch = currentLesson.title.toLowerCase();
        const wasteType = typeMatch === "hazardous" ? "special" : typeMatch;
        return bins.find(bin => bin.type === wasteType);
    };
        ...
        <div className="page-container">
            ...
            <main className="auth-card" style={{ maxWidth: "800px", width: "95%" }}>
                ...
                <div className="lesson-content-scroll">
                    ...
                    {/* Visual display of the correct bin for each respective lesson */}
                    {currentBin && (
                        <div style={{ textAlign: "center", marginBottom: "30px" }}>
                            <img 
                                src={currentBin.closed} 
                                alt={currentBin.name} 
                                style={{ 
                                    width: "120px", 
                                    height: "auto", 
                            ...
                            <p style={{ 
                                ...
                                USE THE {currentBin.name.toUpperCase()} BIN
                            </p>
                        </div>
                    )}

                    <p style={{ fontSize: "1.1rem", marginBottom: "2rem" }}>{currentLesson?.content}</p>
                ...
                {/*Navigation buttons for lessons and complete action*/}    
                <div className="lesson-navigation-bar">
                    <button className="nav-arrow" disabled={!prevPath} onClick={() => navigate(`/lessons/${prevPath}`)}> ← PREV </button>
